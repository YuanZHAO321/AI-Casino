...
export function rulesLayer(view: Pick<TableView, 'rules'>): string {
  const r = view.rules
  return [
    `你正坐在一张英式 21 点（Blackjack）赌桌旁。规则：`,
    `- ${r.decks} 副牌混洗成一靴，切牌到达后局间重洗。所有牌明发：玩家两张明牌，庄家先发一张明牌，所有人行动完庄家才补牌（无暗牌，无保险）。`,
    `- 庄家补牌到至少 17 点${r.hitSoft17 ? '（软 17 继续要牌）' : '（软 17 停牌）'}。Blackjack（最初两张 A+10）赔 3:2，普通赢 1:1，平局退注。庄家两张成 Blackjack 时通吃所有注（对方 Blackjack 平局）。`,
    `- Double：任意前两张可加倍，只买一张。Split：仅前两张同点对子可分一次，不可再分${r.splitAcesOneCard ? '，分 A 每手只补一张' : ''}；分 A 后 A+10 只算 21 不算 Blackjack。${r.doubleAfterSplit ? '分牌后可加倍。' : '分牌后不可加倍。'}`,
    ...
    `你只能看到桌面上公开的信息（各家明牌、注额、已发牌情况），像一个真实玩家一样思考。`
  ].join('\n')
}
  ...
  const roleDesc =
    persona.role === 'opponent'
      ...
      : persona.role === 'companion'
        ? '你是坐在玩家身边的陪玩，看着玩家的牌和桌面替 TA 出谋划策、吐槽喝彩。你自己不下注。'
...
const SPEECH_FORMAT = `直接输出你要说的话本身，不要 JSON，不要引号包裹，不要任何前缀。`
...
export function serializeView(view: TableView, opts: { cardCounting: boolean }): string {
  ...
  const d = view.dealer
  lines.push(`庄家: ${d.cards.length ? `${d.cards.join(' ')}(${d.total}${d.blackjack ? ' BJ' : d.bust ? ' 爆' : ''})` : '未发牌'}`)
  for (const s of view.seats) {
    const you = s.isYou ? '（你）' : s.isHuman ? '（人类玩家）' : ''
    const side = sideBetsBrief(s.sideBets)
    lines.push(`${s.name}${you}: ${handLine(s)}${side}${settledBrief(s)}`)
  }
  if (opts.cardCounting) {
    const seen = view.seen
    const ranks = Object.entries(seen.byRank).map(([r, n]) => `${r}:${n}`).join(' ')
    lines.push(`本靴已发 ${seen.dealt}/${seen.dealt + seen.remaining} 张，已见点数 [${ranks}]`)
  }
  return lines.join('\n')
}
...
export const SPEECH_INSTRUCTIONS = {
  ...
  companionAdvice: '玩家想听你的建议。结合玩家现在的牌和庄家明牌，给出你建议的打法（要牌/停牌/加倍/分牌）和简短理由，用你的角色语气说。',
